...
export default function ArticleCard({ article, onClick }: { article: Article; onClick?: (article: Article) => void }) {
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--color-surface)'; if (!isMobile) e.currentTarget.style.borderLeftColor = 'var(--color-accent)' }}
      onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; if (!isMobile) e.currentTarget.style.borderLeftColor = 'transparent' }}
    >
      <div style={{ display: 'flex', gap: isMobile ? 6 : 12, alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
              ...
              {article.key_terms?.slice(0, 3).map((term) => (
                <Tooltip key={term} title={term}>
                  <Tag style={{ fontSize: 8, borderRadius: 0, margin: 0, padding: '0 5px', maxWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
                    {term}
                  </Tag>
                </Tooltip>
              ))}
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ flexShrink: 0, paddingTop: isMobile ? 0 : 2 }}>
          <Button
            ...
            style={{ color: bookmarked ? '#ef4444' : 'var(--color-text-tertiary)', width: isMobile ? 22 : 26, height: isMobile ? 22 : 26 }}
          />
        </div>
      </div>
    </div>
