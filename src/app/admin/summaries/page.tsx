  ...
  const columns = [
    {
      ...
      render: (s: string) => {
        ...
        return (
          <Tag
            color={SOURCE_COLORS[s] || 'default'}
            style={{ background: bg[s] || '#6b7280', color: '#fff', border: 'none' }}
          >
            {SOURCE_LABELS[s] || s}
          </Tag>
        );
    ...
    },
    {
      ...
      render: (h: string, record: ArticleWithSummary) => (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          style={{ fontWeight: 500, fontSize: 13, color: '#fafafa' }}
        >
          {h}
        </a>
      ),
    },
    {
      ...
      render: (s: string | null) => {
        if (!s)
          return (
            <Tag color="warning" style={{ fontSize: 11, background: '#f59e0b', color: '#fff', border: 'none' }}>
              Not generated
            </Tag>
          );
        return (
