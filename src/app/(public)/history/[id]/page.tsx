...
export default function HistoryDetailPage() {
  ...
  return (
    <div>
      <div className="article-card" style={{ padding: 22, marginBottom: 28 }}>
        <Title level={4} style={{ margin: 0, marginBottom: 18, fontSize: 16 }}>
          {quiz.title || 'Quiz Details'}
        </Title>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 14 }}>
          <div>
            <Text style={{ fontSize: 11, display: 'block', marginBottom: 4, color: '#bbb', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Score</Text>
            <div style={{
          ...
          <div>
            <Text style={{ fontSize: 11, display: 'block', marginBottom: 4, color: '#bbb', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Date</Text>
            <Text style={{ fontSize: 14 }}>
              {new Date(quiz.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
            </Text>
          </div>
          <div>
            <Text style={{ fontSize: 11, display: 'block', marginBottom: 4, color: '#bbb', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Time</Text>
            <Text style={{ fontSize: 14 }}>
              {quiz.time_taken_sec ? `${Math.floor(quiz.time_taken_sec / 60)}m ${quiz.time_taken_sec % 60}s` : 'N/A'}
            </Text>
          </div>
          <div>
            <Text style={{ fontSize: 11, display: 'block', marginBottom: 4, color: '#bbb', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Articles</Text>
      ...
      {quiz.articles && quiz.articles.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <Text strong style={{ fontSize: 15, display: 'block', marginBottom: 12 }}>Linked Articles</Text>
      ...
      <div>
        <Text strong style={{ fontSize: 15, display: 'block', marginBottom: 14 }}>Questions &amp; Answers</Text>
